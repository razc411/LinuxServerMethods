...
*/
ClientInstance::ClientInstance(int port, std::string host, int d_size, std::ofstream log, std::mutex mtx):hostname(host), 
	sport(port), data_size(d_size), logfile(log), m_log(mtx) 
{
...
*/
long ClientInstance::get_elapsed_time(struct timeval start)
{
	struct timeval end;
